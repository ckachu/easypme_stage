...
class l10n_pf_account_vat_declaration(models.Model):
    ...
    def _get_period_from(self, cr, uid, ids, context=None):
        ...
        if company.regime_vat == 'real':
            ...
            #Cas 2: Par Mois
            if company.period_declaration == 'month':
                if first_month:
        ...
        if company.regime_vat == 'simplified':
            # Acompte
            if today.month == 9:
            ...
            for field in ['exports_ids', 'others_ids', 'reduced_rate_ids', 'intermediate_rate_ids', 'normal_rate_ids',
                ...
                if (decl.company_regime == 'real') or (decl.company_regime == 'simplified' and decl.type_simplified == 'annual'):
                    ...
                    if field == 'exports_ids':
                        for i in (ac for ac in decl.company_id.exports_ids if ac is not None):
                            res = res + i.balance
                    ...
                    if field == 'others_ids':
                        for i in (ac for ac in decl.company_id.others_ids if ac is not None):
                            res = res + i.balance
                    ...
                    if (decl.type_simplified == 'annual') or (decl.company_vat_type == 'bills'):
                        if field == 'reduced_rate_ids':
                            ...
                            for i in (tx for tx in self.browse(cr, uid, ids, context=ctx_n).company_id.tax_reduced_rate_ids if tx is not None):
                            ...
                            my_list = []
                            # On récupère tous les comptes qui se trouvent dans la Taxe
                            for i in (tx for tx in self.browse(cr, uid, ids, context=ctx_n).company_id.tax_normal_rate_ids if tx is not None):
                                my_list = my_list + list(i.account_collected_id) + list(i.account_paid_id)
                            ...
                            my_list = []
                            # On récupère tous les comptes qui se trouvent dans la Taxe
                            for i in (tx for tx in self.browse(cr, uid, ids, context=ctx_n).company_id.tax_immo_ids if tx is not None):
                                my_list = my_list + list(i.account_collected_id) + list(i.account_paid_id)
                            ...
                            my_list = []
                            # On récupère tous les comptes qui se trouvent dans la Taxe
                            for i in (tx for tx in self.browse(cr, uid, ids, context=ctx_n).company_id.tax_others_goods_services_ids if tx is not None):
                                my_list = my_list + list(i.account_collected_id) + list(i.account_paid_id)
                        ...
                        if field == 'intermediate_rate_ids':
                            ...
                            for i in (ac for ac in self.browse(cr, uid, ids, context=ctx_n).company_id.customers_ids if ac is not None):
                                compte_client_n = compte_client_n + i.balance

                            # on recupere le montant du 706 pour l'annee N
                            for i in (ac for ac in self.browse(cr, uid, ids, context=ctx_n).company_id.turnover_ids if ac is not None):
                            ...
                            for i in (ac for ac in self.browse(cr, uid, ids, context=ctx_open).company_id.customers_ids if ac is not None):
                ...
                # Cas régime simplifié (Acompte)
                    ...
                    if field == 'sales_ids':
                        for i in (ac for ac in decl.company_id.sales_ids if ac is not None):
                            res = res + i.balance

                        decl.update({'excluding_vat_sales': -res})
                    elif field == 'services_ids':
                        for i in (ac for ac in decl.company_id.services_ids if ac is not None):
                            res = res + i.balance
    ...
    name = fields.Char(string='Declaration name', required=True, states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    date_declaration = fields.Date('Declaration date', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    account_id = fields.Many2one('account.account', 'Account')
    ntahiti = fields.Char(string='No Tahiti', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_id = fields.Many2one('res.company', 'Company', required=True, states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_activity = fields.Char(string='Activity', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_phone = fields.Char(string='Telephone', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_street = fields.Char(string='Street', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_city = fields.Char(string='City', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_country = fields.Char(string='Country', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_email = fields.Char(string='Email', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_zip = fields.Char(string='ZIP', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    company_bp = fields.Char(string='BP', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    city_zip = fields.Char(string='City ZIP', states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
    ...
    place = fields.Char(string='Signature Place', required=True, states={'done': [('readonly', True)], 'simulate': [('readonly', True)]})
